...
<body>
	...
	<form class="row g-3" action="EditCompanyServlet?id=<%=id%>"
		method="post">
	...
	List<Shuttle> shuttle = new ArrayList<>();
	shuttle = shuttleService.readShuttleTimeAndDate(name);
	%>

	<form id="shuttle" class="row g-3" action="" method="post">

		<h1>Shuttle Info</h1><div class="col-4">
    <button type="submit" class="btn btn-primary add"><a href="addShuttle.jsp?title=<%=name %>">Add</a></button>
  </div>
		...
		<table class="table table-bordered">
			...
			<tbody>
				...
				<%
				for (Shuttle i : shuttle) {
				%>
				<tr>
					<td><%=i.getShuttleId()%></td>
					<td><%=i.getDate()%></td>
					<td><%=i.getTime()%></td>
					<td><a href="DeleteShuttleServlet?id=<%=i.getShuttleId()%>">Delete</a></td>
				</tr>
				<%
				}
				%>
